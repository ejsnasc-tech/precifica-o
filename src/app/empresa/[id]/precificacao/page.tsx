...
export default function PrecificacaoPage() {
  ...
  const calc = config ? calcular(ingList, margemEfetiva, config, parseFloat(overFunc) || 0, parseFloat(overGv) || 0, parseFloat(overPerdas) || 0) : null;
  ...
  function gerarPDF(p?: Produto & { ingredientes: Ingrediente[] }) {
    const ingListPDF = p ? p.ingredientes : ings.map((i, idx) => ({ ...i, id: idx }));
    const mg = p ? p.margem : margem;
    if (!config) return;
    const c = calcular(ingListPDF, mg, config, parseFloat(overFunc) || 0, parseFloat(overGv) || 0, parseFloat(overPerdas) || 0);
    const nome = p ? p.nome : nomeProd;
    const linhasIng = ingListPDF.map((i) => {
      const sub = calcSub(i);
    ...
    const linhasPag = [
      { l: "💳 Débito", v: c.debito, t: config.taxa_debito }, { l: "💳 Crédito", v: c.credito, t: config.taxa_credito },
      { l: "💰 PIX", v: c.pix, t: config.taxa_pix }, { l: "💵 Dinheiro", v: c.dinheiro, t: config.taxa_dinheiro },
      ...
      <div className={`bg-gradient-to-r ${empresa.cor} p-5 shadow-md`}>
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl font-extrabold text-white">🧮 Precificação</h1>
          </div>
        </div>
      </div>
                ...
                <div className="col-span-2 relative">
                  <input value={iNome}
                    onChange={(e) => {
                      ...
                      if (q.length >= 1) { setSugestoes(catalogo.filter((c) => c.nome.toLowerCase().includes(q))); setMostrarSugestoes(true); }
                    ...
                    <div className="absolute top-full left-0 right-0 z-20 bg-white border border-slate-200 rounded-xl shadow-lg mt-1 overflow-hidden">
                      {sugestoes.map((s) => (
                        <button key={s.id} type="button"
                          onMouseDown={() => { setINome(s.nome); setICusto(String(s.custo_por_unidade)); setIUn(s.unidade); setMostrarSugestoes(false); }}
              ...
              {ings.length > 0 && (
                <table className="w-full text-sm mt-4">
                  <thead><tr className="text-slate-500 border-b">
                    <th className="text-left py-2">Ingrediente</th><th className="text-right py-2">Custo base</th>
                    <th className="text-right py-2">Qtd</th><th className="text-right py-2">Subtotal</th><th></th>
                  </tr></thead>
                  <tbody>
                    {ings.map((i) => (
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
                  <button onClick={() => setModoCalc("margem")} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${modoCalc === "margem" ? "bg-white shadow text-slate-800" : "text-slate-500"}`}>Margem %</button>
                  <button onClick={() => setModoCalc("preco")} className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${modoCalc === "preco" ? "bg-white shadow text-slate-800" : "text-slate-500"}`}>Preço de venda</button>
                </div>
              </div>
              {modoCalc === "margem" ? (
                <>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input type="number" step="0.01" value={precoVenda} onChange={(e) => setPrecoVenda(e.target.value)} placeholder="Ex: 25,00"
                  ...
                  {parseFloat(precoVenda) > 0 && (
                    <div className={`rounded-xl p-3 text-sm font-semibold ${margemEfetiva >= 0 ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}>
                      {margemEfetiva >= 0 ? `✅ Margem implícita: ${margemEfetiva.toFixed(1)}%` : `⚠️ Preço abaixo do custo — margem negativa (${margemEfetiva.toFixed(1)}%)`}
                    </div>
                  )}
                </div>
              )}
            ...
            {calc && (
              <div className="bg-white rounded-2xl shadow p-6">
                ...
                <div className="flex gap-3">
                  <button onClick={salvarProduto} disabled={salvando} className="bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white font-bold px-6 py-3 rounded-xl">
                    {salvando ? "Salvando..." : editandoProduto ? "💾 Salvar alterações" : "💾 Salvar produto"}
                  </button>
                  <button onClick={() => gerarPDF()} className="bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-3 rounded-xl">🖨️ PDF</button>
                  {editandoProduto && <button onClick={limparForm} className="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold px-6 py-3 rounded-xl">Cancelar</button>}
                </div>
              </div>
            )}
          </div>
        ...
        {aba === "lista" && (
          <div className="space-y-4">
            ...
            {produtos.map((p) => {
              ...
              const ingListItem = prodDetail?.id === p.id ? (prodDetail.ingredientes ?? []) : [];
              const cProd = config ? calcular(ingListItem, p.margem, config, funcPorProd, gastosPorProduto, config.perdas_pct) : null;
              return (
                <div key={p.id} className="bg-white rounded-2xl shadow p-6">
                  <div className="flex justify-between items-start mb-4 flex-wrap gap-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button onClick={() => isOpen ? setProdDetail(null) : loadProdDetail(p)} className="bg-slate-100 text-slate-600 hover:bg-slate-200 px-3 py-1 rounded-lg text-sm font-semibold">{isOpen ? "Fechar" : "Ver detalhes"}</button>
                      <button onClick={() => iniciarEdicao(p)} className="bg-amber-100 text-amber-700 hover:bg-amber-200 px-3 py-1 rounded-lg text-sm font-semibold">✏️ Editar</button>
                      <button onClick={() => prodDetail?.id === p.id && prodDetail.ingredientes ? gerarPDF(prodDetail as Produto & { ingredientes: Ingrediente[] }) : undefined} className="bg-red-100 text-red-600 hover:bg-red-200 px-3 py-1 rounded-lg text-sm font-semibold">🖨️</button>
                      <button onClick={() => deletarProduto(p.id)} className="bg-slate-100 text-slate-500 hover:bg-red-100 hover:text-red-500 px-3 py-1 rounded-lg text-sm font-semibold">🗑️</button>
                    </div>
                  </div>
                  {isOpen && cProd && (
                    <>
                      <div className="space-y-1 mb-4 text-sm">
                        {[
                          { l: "🧂 Insumos", v: cProd.ins, cor: "" }, { l: `🗑️ Perdas ${config.perdas_pct}%`, v: cProd.perdas, cor: "text-red-500", p: "+ " },
                          { l: "👥 Funcionários", v: cProd.func, cor: "text-red-500", p: "+ " }, { l: `⚡ Gastos Var.`, v: cProd.gv, cor: "text-red-500", p: "+ " },
                          ...
                          <div key={l} className="flex justify-between py-1 border-b border-slate-100">
                            <span className="text-slate-500">{l}</span><span className={`font-semibold ${cor}`}>{pr}{fmt(v)}</span>
                          </div>
                        ))}
                        <div className="flex justify-between py-1 border-b-2 border-slate-300"><span className="font-bold text-slate-700">📦 Custo Total</span><span className="font-bold">{fmt(cProd.custoTotal)}</span></div>
                        <div className="flex justify-between py-1"><span className="font-bold text-green-600">💰 Lucro ({p.margem}%)</span><span className="font-bold text-green-600">{fmt(cProd.lucro)}</span></div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                        {[{ l: `💳 Débito`, v: cProd.debito }, { l: `💳 Crédito`, v: cProd.credito }, { l: `💰 PIX`, v: cProd.pix }, { l: `💵 Dinheiro`, v: cProd.dinheiro }].map(({ l, v }) => (
                          <div key={l} className="bg-white rounded-xl p-2 text-center border border-slate-200">
                            <p className="text-slate-500 text-xs">{l}</p><p className="font-bold text-blue-700 text-sm">{fmt(v)}</p>
                          </div>
                        ))}
                      </div>
                      <details className="text-sm text-slate-400">
                        <summary className="cursor-pointer hover:text-slate-600 font-medium mb-2">🧂 Ingredientes ({ingListItem.length})</summary>
                        <table className="w-full mt-2">
                          <thead><tr className="text-slate-500 border-b"><th className="text-left py-1">Nome</th><th className="text-right py-1">Qtd</th><th className="text-right py-1">Subtotal</th><th></th></tr></thead>
                          <tbody>
                            {ingListItem.map((i) => (
          ...
          <div className="max-w-2xl space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                <input value={novoCat.nome} onChange={(e) => setNovoCat({ ...novoCat, nome: e.target.value })} placeholder="Nome (ex: Farinha de trigo)" className="col-span-2 border border-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
                <input type="number" step="0.01" value={novoCat.custo_por_unidade} onChange={(e) => setNovoCat({ ...novoCat, custo_por_unidade: e.target.value })} placeholder="Custo R$" className="border border-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
                <select value={novoCat.unidade} onChange={(e) => setNovoCat({ ...novoCat, unidade: e.target.value })} className="border border-slate-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                  {["kg","g","L","ml","un","cx","pc"].map((u) => <option key={u}>{u}</option>)}
                </select>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow overflow-hidden">
              {catalogo.length === 0 ? (
                <div className="p-12 text-center"><p className="text-5xl mb-3">🧂</p><p className="text-slate-500 font-medium">Nenhum ingrediente no catálogo ainda.</p></div>
              ) : (
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {catalogo.map((c) => (
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                        {editandoCat?.id === c.id ? (
                          <>
                            <td className="px-5 py-3"><input value={editandoCat.nome} onChange={(e) => setEditandoCat({ ...editandoCat, nome: e.target.value })} className="w-full border border-blue-300 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" /></td>
                            <td className="px-5 py-3 text-right"><select value={editandoCat.unidade} onChange={(e) => setEditandoCat({ ...editandoCat, unidade: e.target.value })} className="border border-blue-300 rounded-lg px-2 py-1 text-sm">{["kg","g","L","ml","un","cx","pc"].map((u) => <option key={u}>{u}</option>)}</select></td>
                          ...
                          <>
                            ...
                            <td className="px-5 py-3 text-right font-semibold text-slate-700">{c.custo_por_unidade.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}/{c.unidade}</td>
                            <td className="px-5 py-3 text-right">
                              <div className="flex gap-2 justify-end">
          ...
          <div className="max-w-2xl space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  <button key={v} onClick={() => setCfgForm({ ...cfgForm, regime: v })}
                    ...
                    <p className="font-bold text-sm">{l}</p><p className="text-xs text-slate-500 mt-1">{sub}</p>
                  </button>
                ))}
              </div>
              {cfgForm.regime === "simples_nacional" && (
                <div className="bg-slate-50 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[{ v: "I", l: "Anexo I — Comércio", sub: "4,0%" }, { v: "II", l: "Anexo II — Indústria", sub: "4,5%" }, { v: "III", l: "Anexo III — Serviços", sub: "6,0%" }, { v: "custom", l: "✏️ Personalizada", sub: "Informe a alíquota" }].map(({ v, l, sub }) => (
                      <button key={v} onClick={() => setCfgForm({ ...cfgForm, anexo: v })}
                        ...
                        <p className="font-bold text-sm">{l}</p><p className="text-xs text-slate-500">{sub}</p>
                      </button>
                    ))}
                  </div>
                  {cfgForm.anexo === "custom" && (
                    <div>
                      ...
                      <input type="number" step="0.1" value={cfgForm.aliquota_custom} onChange={(e) => setCfgForm({ ...cfgForm, aliquota_custom: Number(e.target.value) })} className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
                    </div>
                  )}
              ...
              {(cfgForm.regime === "lucro_presumido" || cfgForm.regime === "lucro_real" || cfgForm.regime === "custom") && (
                <div className="bg-slate-50 rounded-xl p-4 mt-3">
                  ...
                  <input type="number" step="0.1" value={cfgForm.regime === "lucro_presumido" ? (cfgForm.aliquota_custom || 13.33) : cfgForm.regime === "lucro_real" ? (cfgForm.aliquota_custom || 34) : cfgForm.aliquota_custom} onChange={(e) => setCfgForm({ ...cfgForm, aliquota_custom: Number(e.target.value) })} className="mt-1 w-40 border border-slate-300 rounded-lg px-3 py-2 focus:outline-none" />
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[{ l: "💳 Débito (%)", k: "taxa_debito" }, { l: "💳 Crédito à vista (%)", k: "taxa_credito" }, { l: "💰 PIX (%)", k: "taxa_pix" }, { l: "💵 Dinheiro (%)", k: "taxa_dinheiro" }].map(({ l, k }) => (
                  <div key={k}>
                    <label className="text-sm text-slate-600">{l}</label>
                    <input type="number" step="0.01" value={cfgForm[k as keyof Config] as number} onChange={(e) => setCfgForm({ ...cfgForm, [k]: Number(e.target.value) })} className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={cfgForm.funcionarios_custo} onChange={(e) => setCfgForm({ ...cfgForm, funcionarios_custo: Number(e.target.value) })} className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
                ...
                <div>
                  ...
                  <input type="number" value={cfgForm.funcionarios_qtd} onChange={(e) => setCfgForm({ ...cfgForm, funcionarios_qtd: Number(e.target.value) })} className="mt-1 w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <p className="text-slate-400 text-sm mb-4">Adicione cada despesa mensal da empresa</p>
              <div className="space-y-2 mb-4">
                {gastosItens.length === 0 && <p className="text-sm text-slate-400 text-center py-4 bg-slate-50 rounded-xl">Nenhum gasto cadastrado ainda.</p>}
                {gastosItens.map((g) => (
              ...
              {gastosItens.length > 0 && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 space-y-1">
                  <div className="flex justify-between text-sm"><span className="text-slate-600">Total mensal</span><span className="font-bold text-slate-800">{fmt(gastosMensal)}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate-600">Produção mensal ({cfgForm.funcionarios_qtd} un.)</span><span className="text-slate-500">÷ {cfgForm.funcionarios_qtd}</span></div>
                  <div className="flex justify-between text-sm border-t border-yellow-300 pt-1 mt-1"><span className="font-bold text-yellow-800">⚡ Por produto</span><span className="font-bold text-yellow-800">{fmt(gastosPorProduto)}</span></div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow p-6">
              ...
              <input type="number" step="0.1" value={cfgForm.perdas_pct} onChange={(e) => setCfgForm({ ...cfgForm, perdas_pct: Number(e.target.value) })} className="mt-1 w-64 border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" />
            </div>

            <button onClick={salvarConfig} disabled={salvandoCfg} className="w-full bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white font-bold py-4 rounded-2xl text-lg transition-colors">
